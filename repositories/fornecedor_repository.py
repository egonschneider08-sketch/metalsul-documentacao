...
class FornecedorDAO(Conexao):
    ...
    def buscarFornecedor(self, id_fornecedor):
        ...
        sql = "SELECT * FROM empresa.fornecedor WHERE id_fornecedor = %s"
        try:
            self.cursor.execute(sql, (id_fornecedor,))
            resultado = self.cursor.fetchone()
            return resultado
        except Exception as erro:
            print(f"Erro ao buscar fornecedor! Erro: {erro}")
            return None

    def ExcluirFornecedor(self, id_fornecedor):
        """Exclui um fornecedor pelo ID."""
        sql = "DELETE FROM empresa.fornecedor WHERE id_fornecedor = %s"
        try:
            self.cursor.execute(sql, (id_fornecedor,))
            self.conexao.commit()
            ...
            self.conexao.rollback()
            print(f"Erro ao excluir fornecedor! Erro: {erro}")
